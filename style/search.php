...
if($_POST['search'] == 'all_book'){
  $result = $this->preparedQuery("SELECT book.id,book.isbn,book.cover_image,book.title,author.name AS authorName,publisher.name AS publisherName
  ...
  $resultNum = $this->preparedQuery("SELECT book.id,book.isbn,book.cover_image,book.title,author.name AS authorName,publisher.name AS publisherName
                                  FROM books book, authors author, publishers publisher
                                  WHERE book.author_id=author.id AND book.publisher_id=publisher.id AND
                                  (author.name LIKE ?) OR (publisher.name LIKE ?) OR (book.isbn LIKE ?)
                                  OR (title LIKE ?) GROUP BY book.id LIMIT 5",array('%'.$search.'%','%'.$search.'%','%'.$search.'%','%'.$search.'%'),'num');
}elseif($_POST['search'] == 'book_name'){
  $result = $this->preparedQuery("SELECT * FROM books WHERE title LIKE ?",array('%'.$search.'%'));
  $resultNum = $this->preparedQuery("SELECT * FROM books WHERE title LIKE ?",array('%'.$search.'%'),'num');
}elseif($_POST['search'] == 'author'){
  $result = $this->preparedQuery("SELECT book.isbn,book.cover_image,book.title,author.name AS authorName
                                  FROM books book, authors author
                                  WHERE book.author_id=author.id AND author.name LIKE ?",array('%'.$search.'%'));
  $resultNum = $this->preparedQuery("SELECT book.isbn,book.cover_image,book.title,author.name AS authorName
                                  FROM books book, authors author
                                  WHERE book.author_id=author.id AND author.name LIKE ?",array('%'.$search.'%'),'num');
}elseif($_POST['search'] == 'publisher'){
  $result = $this->preparedQuery("SELECT book.id,book.cover_image,book.title,publisher.name AS publisherName
                                  FROM books book, publishers publisher
                                  WHERE book.publisher_id=publisher.id AND publisher.name LIKE ?",array('%'.$search.'%'));
  $resultNum = $this->preparedQuery("SELECT book.id,book.cover_image,book.title,publisher.name AS publisherName
                                  FROM books book, publishers publisher
                                  WHERE book.publisher_id=publisher.id AND publisher.name LIKE ?",array('%'.$search.'%'),'num');
}elseif($_POST['search'] == 'isbn'){
  $result = $this->preparedQuery("SELECT * FROM books WHERE isbn LIKE ?",array('%'.$search.'%'));
  $resultNum = $this->preparedQuery("SELECT * FROM books WHERE isbn LIKE ?",array('%'.$search.'%'),'num');
} ?>
...
<div class="col-12 text-center">
  <h1 class="text-white"><?php echo 'Search for <span style="font-family: sans-serif;">'.$_POST['search_input'].'</span>'; ?></h1>
</div>

<?php if($resultNum == 0){ ?>
  <br>
  <div class="alert alert-danger text-center">No results were found for this search</div>
<?php } ?>
